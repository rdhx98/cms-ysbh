@props(['blockType', 'placeholder' => 'Mulai mengetik...'])
...
<div x-data="tiptap(
  ...
  <div x-ref="editorElement" data-placeholder="{{ $placeholder }}" @click="focusEditor()"
    :class="{
        'min-h-16': '{{ $blockType }}'
        === 'heading',
        'min-h-48': '{{ $blockType }}'
  ...
  <div x-show="showLinkModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm p-4">
    <div @click.outside="cancelLink()" class="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-md p-6 space-y-4">
      ...
      <div>
        ...
        <input type="text" x-model="linkInputUrl" @keydown.enter.prevent="saveLink()" placeholder="https://"
          class="w-full border-gray-300 rounded-lg text-sm shadow-sm focus:ring-blue-500 focus:border-blue-500"> <!-- Atribut autofocus dihapus di sini -->
      </div>
      ...
      <div class="flex justify-end gap-2 pt-2">
        ...
        <button type="button" @click="saveLink()"
          class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition shadow-sm">
          Simpan Tautan
        </button>
      </div>
    </div>
