  ...
  <div x-show="!isCollapsed" x-collapse x-cloak class="p-4 space-y-5 bg-white rounded-b-xl">
        ...
        <div class="flex flex-wrap gap-2">
            @foreach($buttons as $index => $btn)
                <button type="button" @click="activeTab = {{ $index }}"
                    ...
                    <div @click.stop="$wire.set('content.{{ $blockId }}.data.buttons', $wire.get('content.{{ $blockId }}.data.buttons').filter((_, i) => i !== {{ $index }})); activeTab = 0;"
    ...
    <div class="min-h-[200px]">
        @foreach($buttons as $index => $btn)
          ...
          <div x-show="activeTab === {{ $index }}" x-cloak wire:key="button-{{ $blockId }}-{{ $index }}"
               class="p-4 border border-gray-100 bg-gray-50 rounded-xl"
               x-data="{ localStyle: $wire.entangle('content.{{ $blockId }}.data.buttons.{{ $index }}.style').live }">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div class="space-y-4">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="content.{{ $blockId }}.data.buttons.{{ $index }}.label.{{ $code }}" placeholder="Misal: Hubungi Kami" class="text-xs border-gray-200 focus:ring-foresty rounded-md py-1.5 bg-white shadow-sm">
                    </div>

                    <div class="flex flex-col gap-1.5">
                        <label class="text-[10px] font-bold text-foresty uppercase">Tautan (URL)</label>
                        <div class="flex items-center gap-1">
                            <input type="text" wire:model.live="content.{{ $blockId }}.data.buttons.{{ $index }}.url" placeholder="#section atau https://..." class="w-full text-xs border-gray-200 focus:ring-foresty rounded-md py-1.5 bg-white shadow-sm">
                            <button type="button"
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($stylesList as $style)
                            <button type="button"
                                @click="localStyle = '{{ $style['value'] }}'"
                                class="flex flex-col items-start p-2 rounded-lg border transition-all text-left bg-white"
    ...
    <button type="button" @click="
        ...
        {{-- 🌟 Fokus otomatis ke tab yang baru dibuat --}}
