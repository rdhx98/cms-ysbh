...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-200 group/eyebrow" x-data="{
    isCollapsed: false,
    init() {
  ...
  <div class="flex items-center justify-between p-2 bg-gray-100 cursor-pointer select-none transition-all duration-200 group-hover:bg-white" :class="isCollapsed ? 'rounded-xl' : 'rounded-t-xl border-b border-gray-200'">
    ...
    <div class="flex items-center gap-2">
      ...
      <div class="p-1 bg-sage-soft rounded-md">
        <x-dynamic-component :component="'lucide-bookmark'" class="h-4 w-4 text-forest" stroke-width="2.5" />
      </div>
    ...
    {{-- <div class="flex items-center gap-2">
      ...
      <span class="text-xs font-extrabold text-gray-500 uppercase tracking-widest flex items-center gap-1.5">
    ...
    <div class="flex items-center gap-2" @click.stop>
      ...
      <span class="text-xs font-bold text-forest uppercase ml-2 bg-sage-soft px-1.5 py-0.5 rounded shadow-sm">{{ $code }}</span>
    </div>
  </div>
  ...
  <div x-show="!isCollapsed" x-collapse x-cloak class="p-4 space-y-4 bg-gray-50/50 rounded-b-xl" x-data="{
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div class="md:col-span-1" x-data="{ openPicker: false, searchQuery: '' }">
        <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Ikon Global</label>
        ...
        <div class="relative">
          ...
          <div x-show="openPicker" @click.away="openPicker = false" x-cloak style="display: none;" class="absolute left-0 mt-1 w-64 bg-white border border-zinc-200 rounded-xl shadow-xl p-3 z-50 flex flex-col gap-2">

            <input type="text" x-model="searchQuery" placeholder="Cari ikon..." class="w-full text-xs border border-zinc-200 rounded-lg px-2.5 py-1.5 focus:ring-0 focus:border-forest" />
      ...
      <div class="md:col-span-1" x-data="{ openColor: false }">
        <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Warna Global</label>

        <div class="relative">
          <button type="button" @click="openColor = !openColor"
            class="w-full flex items-center justify-between border border-zinc-200 bg-white text-zinc-700 rounded-md py-2 px-3 text-xs shadow-sm hover:bg-zinc-50 transition-colors focus:outline-none">
            <div class="flex items-center gap-2 truncate">
              <span class="w-3.5 h-3.5 rounded-full border border-gray-200 shrink-0" :style="`background-color: ${localColor || '#e05a47'}`"></span>
          ...
          <div x-show="openColor" @click.away="openColor = false" x-cloak style="display: none;" class="absolute left-0 mt-1 w-48 bg-white border border-zinc-200 rounded-xl shadow-xl p-2 z-50 flex flex-col gap-1">

            @foreach ($colorsList as $color)
              <button type="button" @click="setColor('{{ $color['value'] }}'); openColor = false" class="flex items-center gap-2 w-full p-2 text-xs text-left rounded-lg hover:bg-zinc-50 transition-colors"
      ...
      <div class="md:col-span-2">
        <label class="block text-xs font-semibold text-forest uppercase mb-1">Teks Eyebrow ({{ strtoupper($code) }})</label>
        <input type="text" wire:model.live.debounce.300ms="content.{{ $blockId }}.data.text.{{ $code }}" placeholder="Contoh: ARTIKEL & CERITA LAPANGAN"
    ...
    <div class="pt-3 border-t border-dashed border-gray-200">
      <span class="block text-xs font-semibold text-gray-400 uppercase mb-2">Pratinjau Langsung:</span>

      <div class="px-4 py-3 bg-white border border-gray-200 rounded-lg shadow-2xs inline-flex items-center">
        <span class="inline-flex items-center gap-2.5 font-['Instrument_Sans',sans-serif] text-[11px] md:text-[13px] font-bold tracking-[0.16em] uppercase transition-colors" :style="`color: ${localColor || '#e05a47'}`">
