...
<div @toggle-collapse-all.window="isCollapsed = $event.detail" @sync-columns-tab-{{ strtolower($blockId) }}.window="activeTab = $event.detail" @sync-collapse-{{ strtolower($blockId) }}.window="isCollapsed = $event.detail"
  class="bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-200 group/column" x-data="{
      activeTab: 'left',
      isCollapsed: false,
      layoutMode: 'split',
      init() {
  ...
  <div class="bg-gray-50 border-b border-gray-200" :class="isCollapsed ? 'rounded-b-xl' : ''">
    ...
    <div x-show="layoutMode === 'split'" x-cloak class="flex px-4 space-x-2 relative top-[1px]">
      <button @click="activeTab = 'left'; $dispatch('sync-columns-tab-{{ strtolower($blockId) }}', 'left')" type="button"
      ...
      <button @click="activeTab = 'right'; $dispatch('sync-columns-tab-{{ strtolower($blockId) }}', 'right')" type="button"
  ...
  <div x-show="!isCollapsed" x-collapse x-cloak :class="layoutMode === 'single' ? 'grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-200' : 'block'" class="bg-white">
    ...
    <div x-show="layoutMode === 'single' || activeTab === 'left'" class="flex flex-col h-full">
      ...
      <div x-sort x-sort:config="{ group: 'left_zone_{{ $blockId }}', animation: 150, handle: '.child-drag-handle', onEnd: (evt) => updateZoneOrder(evt, 'left_zone') }"
          ...
          @if (isset($allContent[$childId]))
            ...
            <div id="block-wrapper-{{ $childId }}" data-id="{{ $childId }}" x-sort:item="'{{ $childId }}'" wire:key="child-{{ $childId }}"
              ...
              <div class="absolute -top-2.5 -right-2.5 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                <button type="button" wire:click="removeNestedBlock('{{ $blockId }}', 'left_zone', '{{ $childId }}')"
        ...
        <div x-data="{ openDropdown: false }" class="relative">
          ...
          <div x-show="openDropdown" x-cloak class="absolute z-50 bottom-full mb-1 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden">
            <div class="p-1 flex flex-col gap-0.5">
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'left_zone', 'heading'); openDropdown = false"
                class="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 rounded-md transition text-left">Judul (Heading)</button>
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'left_zone', 'paragraph'); openDropdown = false"
              ...
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'left_zone', 'eyebrow'); openDropdown = false"
              ...
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'left_zone', 'image'); openDropdown = false"
    ...
    <div x-show="layoutMode === 'single' || activeTab === 'right'" class="flex flex-col h-full" style="display: none;">
      ...
      <div x-sort x-sort:config="{ group: 'right_zone_{{ $blockId }}', animation: 150, handle: '.child-drag-handle', onEnd: (evt) => updateZoneOrder(evt, 'right_zone') }"
        class="flex-1 p-4 space-y-4 min-h-[150px] bg-gray-50/20">
        @foreach ($rightZone as $childId)
          @if (isset($allContent[$childId]))
            ...
            <div id="block-wrapper-{{ $childId }}" data-id="{{ $childId }}" x-sort:item="'{{ $childId }}'" wire:key="child-{{ $childId }}"
              ...
              <!-- Tombol Hapus -->
              <div class="absolute -top-2.5 -right-2.5 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                <button type="button" wire:click="removeNestedBlock('{{ $blockId }}', 'right_zone', '{{ $childId }}')"
                  class="p-1 bg-red-100 text-red-600 rounded-full hover:bg-red-200 border border-red-200 shadow-sm" title="Hapus Blok Ini">
                  <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path>
                  </svg>
      ...
      <div class="p-4 pt-0 mt-auto">
        <div x-data="{ openDropdown: false }" class="relative">
          ...
          <div x-show="openDropdown" x-cloak class="absolute z-50 bottom-full mb-1 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden">
            <div class="p-1 flex flex-col gap-0.5">
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'right_zone', 'heading'); openDropdown = false"
                class="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 rounded-md transition text-left">Judul (Heading)</button>
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'right_zone', 'paragraph'); openDropdown = false"
                class="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 rounded-md transition text-left">Paragraf</button>
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'right_zone', 'eyebrow'); openDropdown = false"
                class="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 rounded-md transition text-left">Eyebrow</button>
              <button type="button" wire:click="addChildBlock('{{ $blockId }}', 'right_zone', 'image'); openDropdown = false"
                class="w-full flex items-center gap-3 px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 rounded-md transition text-left">Gambar</button>
            </div>
          </div>
        </div>
