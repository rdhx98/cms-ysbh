...
<div class="bg-white border border-gray-200 transition-all duration-300 rounded-xl" x-data="{
  ...
  <div class="bg-white border border-gray-200 transition-all duration-300" :class="isCollapsed ? 'rounded-xl shadow-sm' : 'rounded-xl shadow-md'">
    ...
    <!-- EDITOR BODY -->
    <div x-show="!isCollapsed" x-collapse.duration.500ms>
        ...
        <template x-if="!blockData.template">
          <div class="p-8 relative">
            <div class="text-center mb-10">
              <h3 class="font-display text-xl font-bold text-foresty mb-2">Pilih Desain Kartu</h3>
              <p class="text-sm text-gray-500">Pilih tata letak dasar untuk grup kartu ini. Anda dapat mengubahnya nanti.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              @foreach ($templates as $key => $tpl)
                ...
                {{-- 🌟 1. Tinggi kotak diperbesar menjadi min-h-[260px] (sebelumnya 220px) --}}
                <div @click="setTemplate('{{ $key }}')"
                  class="flex flex-col text-left border-2 border-gray-100 bg-white rounded-xl shadow-sm hover:border-foresty hover:shadow-md transition-all duration-300 group/tpl cursor-pointer overflow-hidden min-h-[260px]">

                  {{-- 🌟 2. Panggung area abu-abu diperbesar menjadi min-h-[190px] --}}
                  <div class="px-4 flex-1 relative flex items-center justify-center bg-gray-50/50 overflow-hidden min-h-[190px]">

                    {{-- MODE DEFAULT: Ikon Besar --}}
                    <div class="absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 group-hover/tpl:opacity-0 group-hover/tpl:scale-75 group-hover/tpl:-translate-y-4">
                      <div class="w-14 h-14 rounded-2xl bg-white shadow-sm text-gray-400 flex items-center justify-center border border-gray-100 group-hover/tpl:text-foresty transition-colors">
                        <x-dynamic-component :component="'lucide-' . $tpl['icon']" class="w-7 h-7" />
                      </div>
                    </div>

                    {{-- 🌟 3. SOLUSI TERPOTONG: Animasi diubah dari scale-[0.65] membesar ke scale-[0.8] (80%) --}}
                    <div
                      class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 scale-[0.65] translate-y-6 transition-all duration-500 group-hover/tpl:opacity-100 group-hover/tpl:scale-[0.8] group-hover/tpl:translate-y-0 px-4">
                      <div class="w-full">
                  ...
                  {{-- AREA BAWAH: LABEL & DESKRIPSI --}}
                  <div class="p-4 border-t border-gray-100 bg-white group-hover/tpl:bg-sage-soft transition-colors mt-auto relative z-10">
                    <div class="font-bold text-foresty text-[13.5px] mb-1 flex items-center gap-2">
                      {{ $tpl['label'] }}
                      <x-dynamic-component component="lucide-arrow-right" class="w-3.5 h-3.5 text-foresty opacity-0 -translate-x-2 transition-all duration-300 group-hover/tpl:opacity-100 group-hover/tpl:translate-x-0" />
                    </div>
                    <div class="text-[11.5px] text-gray-500 leading-relaxed">{{ $tpl['desc'] }}</div>
                  </div>
                </div>
              @endforeach
            </div>
                  ...
                  <div x-show="activeTab === index" x-transition.opacity.duration.200ms class="col-start-1 row-start-1 p-5 border border-gray-100 bg-gray-50 rounded-xl"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div class="flex flex-col gap-1.5 md:col-span-2" x-show="isTemplate(['stats', 'document', 'profile', 'impact', 'partner', 'minimal'])">
                        <label class="text-[10px] font-bold text-foresty uppercase"
                          x-text="blockData.template === 'stats' ? 'Angka Statistik' : (blockData.template === 'profile' ? 'Nama Tokoh' : 'Judul Utama')"></label>
     ...
     ========================================== -->
      <div x-show="blockData.template" x-cloak wire:key="preview-wrapper-{{ $blockId }}" class="relative rounded-2xl overflow-hidden bg-gray-50 border border-gray-200">
        ...
        {{-- 🌟 TRIK MAGIS ALPINE: Diperbarui agar mendeteksi semua jenis tag (div maupun a) --}}
        <style
          x-text="`
							...
							.preview-grid-{{ $blockId }} .grid > *:not(:nth-child(${activeTab + 1})) { 
