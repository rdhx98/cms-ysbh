...
<div x-data="{
    ...
    activeSlot: 'main',
    isPinned: false,
    isCollapsed: false, // 🌟 Tambahkan State Collapse
    pinStyle: '',
    ...
    // 🌟 KEAJAIBAN JS BOUNDING BOX
    togglePin() {
        if (this.isCollapsed) this.isCollapsed = false; // Buka lipatan jika mau dipin
    ...
    // 🌟 FUNGSI TOGGLE LIPATAN

    toggleCollapse() {
        this.isCollapsed = !this.isCollapsed;

        // 1. Matikan pin jika sedang melayang
        if (this.isCollapsed && this.isPinned) {
            this.togglePin();
        }

        // 2. Pancarkan event agar blok bahasa lain (EN/ID) ikut terlipat secara sinkron
        $dispatch('sync-collapse-{{ strtolower($blockId) }}', this.isCollapsed);
    }
}"

  @sync-card-{{ strtolower($blockId) }}.window="
        if ($event.detail) {
            activeCard = $event.detail.card;
            activeSlot = $event.detail.slot;
        }
  "
  @resize.window="if(isPinned) { togglePin(); togglePin(); }"

  {{-- 🌟 EVENT GLOBAL COLLAPSE DIKEMBALIKAN DENGAN PENGAMAN PIN 🌟 --}}
  @sync-collapse-{{ strtolower($blockId) }}.window="
        isCollapsed = $event.detail; 
        if(isCollapsed && isPinned) { togglePin(); }
  "
  @toggle-collapse-all.window="
        isCollapsed = $event.detail; 
        if(isCollapsed && isPinned) { togglePin(); }
  "
  @force-collapse-children.window="
        if ($event.detail.includes('{{ $blockId }}')) { 
            isCollapsed = true; 
            window.blockCollapseState['{{ $blockId }}'] = true; 
            if(isPinned) { togglePin(); }
        }
  "
@force-expand-children.window="
        if ($event.detail.includes('{{ $blockId }}')) { 
            isCollapsed = false; 
            window.blockCollapseState['{{ $blockId }}'] = false; 
        }
  "
class="w-full">
...
<div x-ref="editor"
:style="pinStyle"
class="bg-white border transition-all duration-200 rounded-xl flex flex-col"
x-bind:class="isPinned ? 'border-foresty ring-4 ring-foresty/20 shadow-2xl overflow-hidden' : 'border-gray-200 shadow-md relative h-auto'">
...
<div class="flex items-center justify-between p-2 bg-gray-100 rounded-t-xl border-b border-gray-200 shrink-0">
	...
	<div class="flex gap-4 items-center">
		<!-- 🌟 Sembunyikan kontrol grid saat dilipat agar header tetap ringkas -->
		...
		<div class="flex items-center gap-1 border-l border-gray-300 pl-4">
			...
			<x-dynamic-component component="lucide-pin" class="w-3.5 h-3.5" x-bind:class="isPinned ? 'rotate-45' : ''" />
		</button>
		
		{{-- 🌟 TOMBOL LIPAT (COLLAPSE) --}}
...
<!-- 🌟 BUNGKUSAN LIPATAN (Membungkus Badan Tengah & Live Preview) -->
	...
	<div class="flex-1 flex flex-col min-h-0 p-4">
		...
		<div class="flex-1 flex flex-col items-center justify-center py-10">
			...
			<div class="flex justify-center gap-4">
				<button type="button" wire:click="addCardItem('{{ $blockId }}', 'stack')" @click="syncTabs(0, 'main')"
				class="px-5 py-3 border-2 border-dashed border-gray-300 rounded-xl hover:border-foresty transition-colors flex flex-col items-center gap-3 group outline-none">
			...
			<button type="button" wire:click="addCardItem('{{ $blockId }}', 'media-object')" @click="syncTabs(0, 'middle')"
			class="px-5 py-3 border-2 border-dashed border-gray-300 rounded-xl hover:border-foresty transition-colors flex flex-col items-center gap-3 group outline-none">
...
<div class="shrink-0 flex items-center gap-2 mb-4 border-b border-gray-200 pb-2">
	<div class="flex-1 overflow-x-auto flex gap-2 no-scrollbar pb-1">
		...
		<button type="button" @click="syncTabs({{ $index }}, '{{ ($card['blueprint'] ?? 'stack') === 'stack' ? 'main' : 'middle' }}')"
		class="text-xs font-bold pl-1 pr-2 whitespace-nowrap outline-none">
		Kartu {{ $index + 1 }}
	</button>
	
	<button type="button" wire:click="removeCardItem('{{ $blockId }}', {{ $index }})" @click="syncTabs(Math.max(0, activeCard - 1), 'main')"
	class="p-0.5 hover:bg-red-500 hover:text-white rounded transition-colors outline-none" title="Hapus Kartu">
...
<!-- AREA KOMPONEN -->
<div class="flex-1 min-h-0 bg-gray-50 border border-gray-200 p-4 sm:p-5 rounded-xl"
    ...
    <div x-show="activeCard === {{ $cIndex }}" x-cloak wire:key="card-editor-{{ $blockId }}-{{ $cIndex }}">
      ...
      <div class="flex flex-col gap-3 mb-6 p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
        ...
        <div class="flex items-center gap-1 mt-1">
          <input type="text" wire:model.blur="content.{{ $blockId }}.data.cards.{{ $cIndex }}.container.url" placeholder="https:// atau pilih dari pencarian internal..."
            class="w-full text-xs border-gray-300 rounded py-1.5 focus:ring-foresty shadow-sm">
      ...
      @if (($card['blueprint'] ?? 'stack') === 'media-object')
        <div class="flex gap-2 mb-4 border-b border-gray-200 pb-2">
          <button type="button" @click="activeSlot = 'left'" x-bind:class="activeSlot === 'left' ? 'text-foresty border-foresty' : 'text-gray-400 border-transparent hover:text-foresty'"
            class="px-2 py-1 text-xs font-bold border-b-2 outline-none transition-colors">Area Kiri</button>
          <button type="button" @click="activeSlot = 'middle'" x-bind:class="activeSlot === 'middle' ? 'text-foresty border-foresty' : 'text-gray-400 border-transparent hover:text-foresty'"
            class="px-2 py-1 text-xs font-bold border-b-2 outline-none transition-colors">Area Tengah</button>
          <button type="button" @click="activeSlot = 'right'" x-bind:class="activeSlot === 'right' ? 'text-foresty border-foresty' : 'text-gray-400 border-transparent hover:text-foresty'"
        ...
        <div x-show="activeSlot === '{{ $slotName }}'" x-cloak>
          <div class="space-y-3">
            @foreach ($elements as $elIndex => $el)
              <div wire:key="el-{{ $blockId }}-{{ $cIndex }}-{{ $slotName }}-{{ $elIndex }}" class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm relative group mb-3">
                ...
                @if (($el['type'] ?? 'text') === 'text')
                  <div class="flex justify-between items-center mb-2">
                    ...
                    <label class="flex items-center gap-1.5 cursor-pointer">
                      <input type="checkbox" wire:model.live="content.{{ $blockId }}.data.cards.{{ $cIndex }}.slots.{{ $slotName }}.{{ $elIndex }}.style.is_pill"
...
@if (count($cards) > 0)
  <div class="shrink-0 bg-gray-50 border-t border-gray-200 rounded-b-xl overflow-hidden flex flex-col"
    x-bind:class="isPinned ? 'max-h-[35vh] border-t-2 border-foresty/20' : ''">
