...
<div class="is-nested-container bg-white border border-gray-300 rounded-xl shadow-sm relative" x-data="{
  ...
  <div class="bg-gray-100 border-gray-200 rounded-t-xl" :class="isCollapsed ? 'rounded-b-xl border-b' : ''">
    <div
      class="px-2 min-h-12 py-2 md:py-0 md:h-12 flex flex-wrap md:flex-nowrap items-center justify-between gap-x-4 gap-y-3 rounded-xl">
      <!-- 🌟 BAGIAN KIRI: Tombol Buka/Tutup & Label Blok -->
      <div class="flex items-center gap-2">
        ...
        <button type="button"
          ...
          <x-dynamic-component component="lucide-circle-chevron-down"
            class="w-5 h-5 text-foresty transition-transform duration-200"
            x-bind:class="isCollapsed ? '-rotate-90' : 'rotate-0'" />
        </button>
        <span
          class="text-fluid-xxs font-extrabold text-gray-500 uppercase tracking-widest flex items-center gap-1.5 cursor-pointer select-none"
        ...
        <button type="button"
          ...
          class="ml-2 flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-forest border border-forest rounded text-fluid-xxs font-bold shadow-sm hover:bg-sage-soft transition-colors">
      ...
      <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-2 border-l border-gray-200 pl-4">
            <div class="flex items-center gap-1">
              ...
              <div class="relative group/tooltip flex items-center justify-center">
                <svg class="w-3.5 h-3.5 text-gray-400 cursor-help hover:text-gray-600 transition-colors" fill="none"
                ...
                <div
                  ...
                  <div
                    class="bg-gray-800 text-white text-[10px] leading-relaxed p-2.5 rounded-lg shadow-xl text-center relative">
        ...
        <div class="flex items-center gap-2">
          ...
          <select wire:model.live="content.{{ $blockId }}.data.col_count"
            class="text-xs font-bold text-forest border-gray-300 rounded py-1 pl-2 pr-6 shadow-sm focus:ring-forest focus:border-forest bg-white">
  ...
  <div x-show="!isCollapsed" x-collapse x-cloak
    ...
    <div x-show="layoutMode === 'split'" x-cloak
      class="flex px-4 space-x-1 relative overflow-x-auto scrollbar-hide bg-gray-100 border-b border-gray-200">
      @for ($i = 1; $i <= $colCount; $i++)
    ...
    @for ($i = 1; $i <= $colCount; $i++)
      ...
      <div x-show="layoutMode === 'single' || activeTab === '{{ $zoneKey }}'" class="flex flex-col h-full"
        style="display: none;">
        ...
        <div x-sort
          ...
          @foreach ($zoneBlocks as $childId)
            @if (isset($allContent[$childId]))
              ...
              {{-- Wrapper Mikro Blok --}}
              <div id="block-wrapper-{{ $childId }}" data-id="{{ $childId }}"
                x-sort:item="'{{ $childId }}'" wire:key="child-{{ $childId }}"
                ...
                <!-- Drag Handle -->
                <div
                  class="child-drag-handle absolute -left-2 top-3 opacity-0 group-hover:opacity-100 cursor-move text-gray-300 hover:text-blue-500 z-10 bg-white rounded-full p-0.5 shadow-sm">
                ...
                <div class="absolute -top-2.5 -right-2.5 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  <button type="button"
                    ...
                    class="p-1 bg-red-100 text-red-600 rounded-full hover:bg-red-200 border border-red-200 shadow-sm"
                ...
                <div class="p-0">
                  <x-dynamic-component :component="'blocks.editor.' . str_replace('_', '-', $childBlock['type'])" :block-id="$childId" :code="$code" :block="$childBlock"
        ...
        <div class="p-4 pt-0 mt-auto border-t border-gray-100">
          <div x-data="{ openDropdown: false }" class="relative mt-4">
            ...
            <div x-show="openDropdown" x-cloak
              class="absolute z-50 bottom-full mb-1 left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden">
              <div class="p-1 flex flex-col gap-0.5">
                <button type="button"
                  wire:click="addChildBlock('{{ $blockId }}', '{{ $zoneKey }}', 'heading'); openDropdown = false"
                  ...
                  (Heading)</button>
                <button type="button"
                  wire:click="addChildBlock('{{ $blockId }}', '{{ $zoneKey }}', 'paragraph'); openDropdown = false"
                ...
                <button type="button"
                  wire:click="addChildBlock('{{ $blockId }}', '{{ $zoneKey }}', 'eyebrow'); openDropdown = false"
                ...
                <button type="button"
                  wire:click="addChildBlock('{{ $blockId }}', '{{ $zoneKey }}', 'image'); openDropdown = false"
