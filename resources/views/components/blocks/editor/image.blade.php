...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm transition-all duration-200 group/image" x-data="{
  ...
  <div x-show="!isCollapsed" x-collapse x-cloak class="space-y-4 p-2">
    ...
    <div>
      ...
      <input type="text" wire:model.live.debounce.500ms="content.{{ $blockId }}.data.url" @keydown.stop @paste.stop="handlePaste($event)" placeholder="Atau masukkan tautan gambar (https://...)"
    ...
    {{-- AREA DROPZONE --}}
    <div class="relative w-full rounded-lg overflow-hidden border-2 transition-all duration-200" :class="isDragging ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100'"
      @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="fileDropped($event)">
      ...
      @if (!empty($imageUrl))
        <div class="relative flex flex-col items-center justify-center p-2 group/image min-h-[144px]">
          ...
          <button type="button" @click="$refs.fileInput.click()"
            class="absolute inset-0 bg-black/60 text-white opacity-0 group-hover/image:opacity-100 transition-opacity flex items-center justify-center gap-2 font-semibold text-sm cursor-pointer">
    ...
    <div>
      ...
      <input type="text" wire:model.live.debounce.500ms="content.{{ $blockId }}.data.caption.{{ $code }}" @keydown.stop @paste.stop placeholder="Tulis keterangan gambar di sini..."
