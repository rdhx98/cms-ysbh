...
<section id="{{ $block['anchor'] ?? '' }}" class="w-full py-12" style="background-color: {{ $bgColor }};">
  ...
  <div class="max-w-7xl mx-auto px-5 sm:px-8">
    ...
    <div class="flex flex-col md:grid md:grid-cols-2 gap-8 lg:gap-14 reveal animate-scroll-reveal">
      ...
      <div class="max-w-none {{ $isReverse ? 'order-2 md:order-1' : 'order-1' }}">
        @foreach ($leftZoneIds as $childId)
          @if (isset($allContent[$childId]))
            ...
            <x-dynamic-component :component="'blocks.render.' . str_replace('_', '-', $childBlock['type'])" :data="$childBlock['data']" :lang="$lang" :all-content="$allContent" />
          @endif
        @endforeach
      </div>
