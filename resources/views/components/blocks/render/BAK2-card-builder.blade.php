                ...
                @foreach ($col['children'] ?? [] as $el)
                  ...
                  @elseif ($type === 'icon')
                    ...
                    @elseif ($type === 'accordion')
                      @php
                        $question = $el['data']['content']['question'] ?? '';
                        $answer = $el['data']['content']['answer'] ?? '';
                        $theme = $el['data']['style']['theme'] ?? 'foresty';

                        // Pemetaan kelas Tailwind berdasarkan tema yang dipilih admin
                      ...
                      {{-- Cegah render jika pertanyaan dan jawaban kosong --}}
                      @if (!empty($question) || !empty($answer))
                        <div
                          x-data="{ open: false }"
                          class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-colors duration-300 {{ $hoverBorder }} w-full"
                        >
                          <!-- Baris Pertama: Pemicu (Pertanyaan) -->
                          <button
                            type="button"
                          ...
                          >
                            <div class="border-t border-gray-100 px-5 pb-5 pt-3 text-sm leading-relaxed text-gray-600">
                              {{-- nl2br(e()) memastikan enter/baris baru dari textarea dirender dengan <br>,
                                   tetapi e() memastikan kode HTML berbahaya disaring (XSS Protection) --}}
                              {!! nl2br(e($answer)) !!}
                            </div>
                          </div>
                        </div>
