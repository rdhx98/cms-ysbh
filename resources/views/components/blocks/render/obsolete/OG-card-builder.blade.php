...
@php
  ...
  // Ambil margin bawah dari data, default ke mb-4
  ...
  <div class="grid {{ $gridClass }} {{ $marginBottom }} gap-4 sm:gap-6 w-full">
    @foreach ($items as $item)
      @php
        ...
        // 🌟 LOGIKA PLACEHOLDER KHUSUS PREVIEW EDITOR
        if ($isPreview) {
            $title =
                $title ?:
                match ($template) {
                    'stats' => '99%',
                    'profile' => 'Nama Tokoh',
                    'partner' => 'Nama Mitra / Lembaga',
                    default => 'Judul Utama Kartu',
                };

            $subtitle =
                $subtitle ?:
                match ($template) {
                    'profile' => 'Jabatan / Peran',
                    'minimal', 'impact' => 'Kategori Label',
                    default => 'Teks Subjudul Kecil',
                };

            $desc = $desc ?: 'Ini adalah kutipan sementara. Mulai mengetik di editor untuk melihat perubahan secara langsung.';

            $tagsRaw = !empty(trim($item['tags'][$lang] ?? '')) ? $item['tags'][$lang] : 'Program, 2026';
        } else {
            // Mode Publik: Tampilkan apa adanya
      ...
      @if ($template === 'stats')
        @if ($theme === 'dashed')
          <div class="text-center py-7 px-4 border border-dashed border-gray-400 bg-foresty/90 rounded-[18px] shadow-sm">
        ...
        @elseif($theme === 'boxed')
          <div class="text-center bg-white border border-foresty/15 rounded-[18px] px-4 py-6 shadow-sm">
        ...
        @else
          <div class="bg-white border border-gray-200 rounded-md p-5 text-center shadow-sm">
      ...
      @elseif($template === 'document')
        <a href="{{ $url }}" @if ($url !== '#') target="_blank" @endif
          class="flex items-center gap-4 bg-white border border-foresty/15 rounded-[18px] p-5 shadow-[0_20px_50px_-25px_rgba(6,45,35,0.35)] hover:-translate-y-1 transition-transform duration-300 group h-full">
      ...
      @elseif($template === 'profile')
        <div class="bg-white border border-foresty/15 rounded-[18px] p-6 text-center shadow-[0_20px_50px_-25px_rgba(6,45,35,0.35)] h-full flex flex-col">
      ...
      @elseif($template === 'impact')
        <div class="bg-white border border-gray-200 rounded-md py-5 px-[22px] shadow-sm h-full flex flex-col">
      ...
      @elseif($template === 'partner')
        <a href="{{ $url }}"
          class="aspect-[2/1] bg-white border border-foresty/15 rounded-[18px] flex items-center justify-center text-ink-soft hover:text-foresty hover:border-foresty transition-colors font-bold text-[13px] text-center p-3 shadow-sm">
          {!! nl2br(e($title)) !!}
        </a>

        {{-- 6. TEMPLATE: MINIMAL --}}
      @elseif($template === 'minimal')
        <div class="bg-white border border-foresty/15 rounded-[18px] px-6 py-5 shadow-sm h-full flex flex-col justify-center">
