...
>
  ...
  <div id="main-editor-scroll-area" class="flex-1 min-h-0 overflow-y-auto overflow-x-hidden pt-6 pb-24 px-4 space-y-8 scrollbar-gutter-stable"
    ...
    <div x-show="editorTab === 'meta'" x-cloak class="space-y-8">
      ...
      <div :class="{
        ...
        @foreach($activeLocales as $code)
          <div x-show="(layoutMode === 'single' && singleActiveLang === '{{ $code }}') || (layoutMode === 'split' && splitLanguages.includes('{{ $code }}'))" class="p-5 bg-white border border-gray-200 rounded-xl shadow-sm space-y-4">
        ...
        @foreach($blockOrder as $blockId)
          ...
          @if($block)
            ...
            <div id="block-wrapper-{{ $blockId }}" wire:key="block-{{ $blockId }}" x-sort:item="'{{ $blockId }}'" class="group relative w-full">
              ...
              <!-- Drag Handle -->
              <div class="absolute top-6 -left-6 opacity-0 group-hover:opacity-100 transition-opacity z-20 hidden md:block">
                <button type="button" class="drag-handle " title="Geser Blok"></button>
              </div>
                ...
                <div class="gap-6" :class="{
                  ...
                  @foreach($activeLocales as $code)
                    ...
                    <div x-show="(layoutMode === 'single' && singleActiveLang === '{{ $code }}') || (layoutMode === 'split' && splitLanguages.includes('{{ $code }}'))"
                          class="space-y-3 bg-white rounded-xl border-2 border-transparent group-hover:border-foresty/80 transition-colors">
  ...
  <div x-cloak class="relative z-[100]" @open-preview-panel.window="previewUrl = $event.detail.url; previewOpen = true;" aria-labelledby="slide-over-title" role="dialog"  aria-modal="true"
    x-data="{
      previewOpen: false,
      previewUrl: '',
