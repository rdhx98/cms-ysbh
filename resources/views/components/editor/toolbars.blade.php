...
<div x-data="{ expanded: false, activeTab: 'format' }" class="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 w-full flex flex-col rounded-t-lg">
  ...
  <div class="flex items-center gap-1 overflow-x-auto scrollbar-none border-b border-zinc-200 dark:border-zinc-700 px-2 pt-1.5 shrink-0 bg-white dark:bg-zinc-900/40">
    ...
    <button type="button" @click="activeTab = 'format'" :class="activeTab === 'format' ? 'bg-zinc-200 dark:bg-zinc-800 text-foresty shadow-xs font-semibold' : 'text-zinc-600 hover:bg-zinc-200/60'"
      class="px-3 py-2 text-xs rounded-t-md transition-colors whitespace-nowrap cursor-pointer">
      Teks
    </button>

    <button type="button" @click="activeTab = 'layout'" :class="activeTab === 'layout' ? 'bg-zinc-200 dark:bg-zinc-800 text-foresty shadow-xs font-semibold' : 'text-zinc-600 hover:bg-zinc-200/60'"
      class="px-3 py-2 text-xs rounded-t-md transition-colors whitespace-nowrap cursor-pointer">
      Format
    </button>
  </div>
  ...
  <div class="flex items-center justify-between w-full px-2 md:px-6 min-h-14">
    ...
    {{-- Area Tombol Tools --}}
    <div x-data="{ isMobile: window.matchMedia('(pointer: coarse)').matches }" @resize.window.debounce.100ms="isMobile = window.matchMedia('(pointer: coarse)').matches"
      :class="isMobile
          ?
      ...
      <div x-show="activeTab === 'format'" class="flex items-center gap-1.5 flex-wrap">
        ...
        <div class="hidden md:flex items-center gap-4 p-1 bg-gray-50 border-l border-gray-200 shrink-0 rounded">
          <select id="font-family-select" :value="getCurrentFont()" @change="changeFontFamily($event.target.value)"
            ...
            <option value="default" x-text="'Bawaan Blok (' + baseFontFamily + ')'"></option>

            {{-- 🌟 2. Opsi Font Eksplisit (Agar bisa dipaksakan kapan saja) --}}
            <option value="Plus Jakarta Sans" style="font-family: 'Plus Jakarta Sans', sans-serif;">Plus Jakarta Sans</option>
            <option value="Arial" style="font-family: Arial, sans-serif;">Arial</option>
            <option value="Fraunces" style="font-family: Fraunces, sans-serif;">Fraunces</option>
        ...
        <div x-data="{ openWeightMenu: false }" class="relative flex items-center">
          ...
          <div x-show="openWeightMenu" @click.away="openWeightMenu = false" style="display: none;"
            ...
            <button type="button" @click="setFontWeight('default'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-700 font-normal">
              Bawaan Blok
            </button>
            <button type="button" @click="setFontWeight('300'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-300 font-light">
              Light (300)
            </button>
            <button type="button" @click="setFontWeight('400'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-700 font-normal">
              Regular (400)
            </button>
            <button type="button" @click="setFontWeight('500'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-700 font-medium">
              Medium (500)
            </button>
            <button type="button" @click="setFontWeight('600'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-800 font-semibold">
              Semi-Bold (600)
            </button>
            <button type="button" @click="setFontWeight('700'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-zinc-900 font-bold">
              Bold (700)
            </button>
            <button type="button" @click="setFontWeight('900'); openWeightMenu = false" class="px-3 py-1.5 text-xs text-left rounded hover:bg-zinc-100 text-black font-black">
              Black (900)
            </button>
          </div>
        </div>
        ...
        <div x-data="{ openColorMenu: false }" class="relative flex items-center border-l border-zinc-200 pl-2 ml-1">
          <button type="button" @click="openColorMenu = !openColorMenu"
            class="flex items-center gap-2 p-1.5 h-9 transition rounded cursor-pointer hover:bg-zinc-200 text-gray-700 bg-zinc-50 shadow-sm border border-transparent" title="Warna Teks">
            <x-dynamic-component component="lucide-palette" class="h-4 w-4" stroke-width="2.5" />
            <div class="w-6 h-6 rounded border border-zinc-300 shadow-inner transition-colors" {{-- :style="updatedAt && getEditor()?.getAttributes('textStyle').color ? { backgroundColor: getEditor()?.getAttributes('textStyle').color } : { backgroundColor: '#18181b' }" --}} :style="{ backgroundColor: getCurrentColor() }">
            </div>
          </button>
          ...
          <div x-show="openColorMenu" @click.away="openColorMenu = false" style="display: none;"
            ...
            <div>
              ...
              <div class="flex gap-2">
                <button type="button" @click="runCommand('setColor', '#000000'); openColorMenu = false"
                  class="w-6 h-6 cursor-pointer hover:border hover:border-coral-muted rounded-full bg-[#000000] hover:scale-110 transition-transform shadow-lg" title="Black"></button>
                <button type="button" @click="runCommand('setColor', '#FFFFFF'); openColorMenu = false"
                  class="w-6 h-6 cursor-pointer hover:border hover:border-gray-600 rounded-full bg-[#FFFFFF] hover:scale-110 transition-transform shadow-lg" title="White"></button>
                <button type="button" @click="runCommand('setColor', '#064F3B'); openColorMenu = false"
                  class="w-6 h-6 cursor-pointer hover:border hover:border-coral-muted rounded-full bg-[#064F3B] hover:scale-110 transition-transform shadow-lg" title="Forest"></button>
                <button type="button" @click="runCommand('setColor', '#EBCC26'); openColorMenu = false"
                  class="w-6 h-6 cursor-pointer hover:border hover:border-gray-600 rounded-full bg-[#EBCC26] hover:scale-110 transition-transform shadow-lg" title="Gold"></button>
                <button type="button" @click="runCommand('setColor', '#E42326'); openColorMenu = false"
                  class="w-6 h-6 cursor-pointer hover:border hover:border-gray-600 rounded-full bg-[#E42326] hover:scale-110 transition-transform shadow-lg" title="Coral"></button>
              </div>
            </div>
            <hr class="border-zinc-100">
            <div>
              <span class="text-[11px] font-bold text-zinc-400 uppercase tracking-wider mb-2 block">Warna Bebas</span>
              <input type="color" :value="getCurrentColor()" @input="runCommand('setColor', $event.target.value)" class="w-full h-8 p-0 border-0 rounded cursor-pointer bg-transparent">
            </div>
            <hr class="border-zinc-100">
            <button type="button" @click="runCommand('unsetColor'); openColorMenu = false"
              class="flex items-center gap-2 px-2 py-2 -mx-1 text-sm font-medium text-red-600 hover:bg-red-50 hover:text-red-700 rounded-md transition-colors text-left" title="Kembalikan ke warna default">
      ...
      <div x-show="activeTab === 'layout'" class="flex items-center gap-1.5 flex-wrap" style="display: none;">
        ...
        <div class="flex items-center gap-1 shrink-0">
          ...
          <div class="relative inline-block">
            <button type="button" @click="toggleEyebrowIconMenu()" :class="checkButtonActive('eyebrow') ? 'bg-coral-dark/10 text-coral-dark' : 'text-gray-600 hover:bg-gray-100'"
              class="flex items-center gap-1 rounded-md px-2 py-1.5 transition-colors" :aria-expanded="isEyebrowIconOpen" aria-haspopup="true" title="Eyebrow">
            ...
            <div x-show="isEyebrowIconOpen" x-cloak x-transition.origin.top.left @click.outside="isEyebrowIconOpen = false" @keydown.escape.window="isEyebrowIconOpen = false"
              ...
              <template x-for="item in eyebrowIcons" :key="item.key">
                <button type="button" @click="selectEyebrowIcon(item.key)"
                  ...
                  class="flex items-center justify-center rounded-md p-2 transition-colors [&>svg]:w-full [&>svg]:h-full w-8 h-8" :title="item.label" x-html="item.svg"></button>
              </template>
            </div>
          </div>
        ...
        <div class="flex items-center gap-1 shrink-0">
          <button type="button" @click="openInternalLinkModal();" :class="checkButtonActive('link', {}, 'default') ? 'bg-sage-soft text-forest font-semibold shadow-sm' : 'text-gray-600'"
    ...
    <div class="flex items-center gap-1.5 pl-3 border-l border-zinc-200 dark:border-zinc-700 shrink-0 bg-zinc-50 dark:bg-zinc-800 z-10 py-1.5">
      <button type="button" @click="expanded = !expanded" class="md:hidden p-1.5 bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 text-zinc-600">
